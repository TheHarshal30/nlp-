...
class EmbeddingAdapter(nn.Module):
    """
    Residual adapter applied in embedding space before the projection head.

    This allows `--freeze_embedding` to keep the base table fixed during
    optimisation while still producing an aligned embedding table at export.
    """
...
def train(args):
    ...
    # ── model components ──────────────────────────────────────────────────────
    adapter = EmbeddingAdapter(embed_dim).to(device)
    ...
    if len(dataset) < args.batch_size:
        raise ValueError(
            f'Not enough pairs for one batch: {len(dataset):,} pairs '
            f'with batch_size={args.batch_size}. Lower --batch_size or extract more pairs.'
        )
    loader  = DataLoader(
        dataset,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for anchors, positives in pbar:
            ...
            z_a_raw = adapter(z_a_raw)
            z_b_raw = adapter(z_b_raw)
    ...
    adapter.eval()
    with torch.no_grad():
        base_matrix = embedding.weight
        aligned_matrix = adapter(base_matrix).cpu().numpy()  # (V, dim)
